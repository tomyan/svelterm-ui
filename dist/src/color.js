...
export function parseColor(input) {
    ...
    if (s in NAMED_COLORS)
        return parseHex(NAMED_COLORS[s]);
...
export function formatHex(c) {
    ...
    if (c.a < 1)
        return hex + toHex(Math.round(c.a * 255));
    return hex;
}
export function formatRgb(c) {
    if (c.a < 1)
        return `rgb(${c.r} ${c.g} ${c.b} / ${round(c.a)})`;
    return `rgb(${c.r} ${c.g} ${c.b})`;
}
export function formatHsl(c) {
...
// --- Named colour lookup ---
export function findNamedColor(c) {
    const hex = '#' + toHex(c.r) + toHex(c.g) + toHex(c.b);
